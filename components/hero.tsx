...
const benefitIcons = {
  hotel: 'M4 21V3h12v18M2 21h20M16 9h4v12M8 7h4M8 11h4M8 15h4M8 21v-3h4v3',
  secure: 'M12 3 4 6v6c0 5 8 9 8 9s8-4 8-9V6l-8-3ZM8 12l3 3 5-6',
  noaccount: 'M14 8a4 4 0 1 1-8 0 4 4 0 0 1 8 0ZM2 21v-2a6 6 0 0 1 10-4M15 18l2 2 5-6'
};
    ...
    <section className="hero shell pb-2 pt-8 sm:pb-4 sm:pt-10 lg:pt-12" aria-labelledby="hero-heading">
      <div className="grid items-center gap-8 md:grid-cols-2 lg:gap-12">
        <div className="min-w-0">
          ...
          <h1 id="hero-heading" className="mt-3 max-w-xl text-4xl font-bold tracking-[-.02em] sm:text-5xl">{t('hero.title')}</h1>
          <p className="mt-4 text-lg font-medium">{t('hero.subtitle')}</p>
          <p className="mt-2 max-w-lg text-base leading-7 text-black/65">{t('hero.description')}</p>
          <div className="hero-actions mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Link className="button-primary" href="#ready-kits">{t('hero.shop')}</Link>
            <Link className="button-secondary" href="#build-your-kit">{t('hero.build')}</Link>
          </div>
          <div className="hero-benefits mt-5 flex flex-wrap gap-2">
            <div className="inline-flex items-center gap-2 rounded-xl border border-black/5 bg-white px-3 py-2 text-xs text-black/65">
              <svg className="h-4 w-4 shrink-0 text-[#184f3a]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d={benefitIcons.hotel} />
              </svg>
              {t('hero.benefits.hotelDelivery')}
            </div>
            <div className="inline-flex items-center gap-2 rounded-xl border border-black/5 bg-white px-3 py-2 text-xs text-black/65">
              <svg className="h-4 w-4 shrink-0 text-[#184f3a]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d={benefitIcons.secure} />
              </svg>
              {t('hero.benefits.securePayment')}
            </div>
            <div className="inline-flex items-center gap-2 rounded-xl border border-black/5 bg-white px-3 py-2 text-xs text-black/65">
              <svg className="h-4 w-4 shrink-0 text-[#184f3a]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d={benefitIcons.noaccount} />
              </svg>
              {t('hero.benefits.noAccount')}
            </div>
          </div>
        </div>
        <div
